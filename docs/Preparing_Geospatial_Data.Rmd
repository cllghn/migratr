...
First, wrap the geospatial functions into programmatic solutions:

```{r}
get_country_provinces <- function(iso3_code) {
  if (!is.character(iso3_code)) {
    stop("iso3_code must be a character",
         call. = FALSE)
  }
  sf::st_as_sf(
    raster::getData(
      "GADM", country = iso3_code, level = 1
      )
               )
}

get_country_cities <- function(iso3_code) {
  ### TO DO:
  # Match the city df with the ISO code for subsetting
  ###
}
```
